...
export class AvailableElementCategoryDto {
  @ApiProperty({ example: 'a1b2c3d4-e5f6-7890-abcd-ef1234567890', description: 'Category UUID v4' })
  id!: string;
...
export class AvailableElementResponseDto {
  ...
  @ApiPropertyOptional({ example: 'https://example.com/icon.png', nullable: true, description: 'Element icon URL' })
  iconUrl!: string | null;

  @ApiProperty({ enum: ElementType, example: ElementType.CONCEPT, description: 'Element type classification' })
  elementType!: ElementType;

  @ApiProperty({ example: true, description: 'Whether element is a starter element available to all users' })
  isStarter!: boolean;

  @ApiProperty({ type: AvailableElementCategoryDto, description: 'Associated active category' })
