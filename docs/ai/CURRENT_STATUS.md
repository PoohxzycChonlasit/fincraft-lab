...
- **P9_WORKSPACE_GRAPH_API_1A contract planning**: FROZEN & RECONCILED (`docs/ai/plans/CANVAS_SNAPSHOT_API_PLAN.md` — `FROZEN_CANVAS_SNAPSHOT_API_CONTRACT_V1`). Defines `GET /workspaces/:id/canvas` and `PUT /workspaces/:id/canvas` for loading and replacing complete Canvas graph snapshots atomically. Final reconciliation incorporates: public response uses `workspaceUpdatedAt` (Workspace last-modified time, not exclusive Canvas timestamp); Element object embeds current public display metadata; two-layer payload limit with 1 MB framework ceiling (`NestExpressApplication.useBodyParser('json', { limit: '1mb' })`) and 512 KB Canvas aggregate domain limit (`MAX_CANVAS_SNAPSHOT_BYTES = 524288`); 400 Bad Request applies between 512 KB and 1 MB, framework 413 Payload Too Large applies beyond 1 MB.
...
- **Canvas Graph Snapshot Contract**: Frozen & reconciled in `docs/ai/plans/CANVAS_SNAPSHOT_API_PLAN.md` (`FROZEN_CANVAS_SNAPSHOT_API_CONTRACT_V1`). Defines `GET /workspaces/:id/canvas` (load canvas graph with embedded Element display object) and `PUT /workspaces/:id/canvas` (replace complete graph atomically via `WorkspaceCanvasService`). Response uses `workspaceUpdatedAt`, Element object contains current public display metadata, two-layer size policy with 1 MB framework ceiling (`NestExpressApplication.useBodyParser('json', { limit: '1mb' })`) and 512 KB domain ceiling (`MAX_CANVAS_SNAPSHOT_BYTES = 524288`). Source implementation not started.
