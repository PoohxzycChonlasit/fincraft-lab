...
- **Available Elements Read API Plan**: Frozen in `docs/ai/plans/AVAILABLE_ELEMENTS_API_PLAN.md` (`FROZEN_AVAILABLE_ELEMENTS_READ_API_CONTRACT_V1`). Defines `GET /elements` contract for loading active Starter Elements + user-unlocked active non-Starter Elements with zero DB writes, public category object, and deterministic ordering (`category.sortOrder ASC`, `isStarter DESC`, `name ASC`).
