...
- **P9_WORKSPACE_GRAPH_API_1A contract planning**: FROZEN & RECONCILED (`docs/ai/plans/CANVAS_SNAPSHOT_API_PLAN.md` — `FROZEN_CANVAS_SNAPSHOT_API_CONTRACT_V1`). Defines `GET /workspaces/:id/canvas` and `PUT /workspaces/:id/canvas` for loading and replacing complete Canvas graph snapshots atomically. Final reconciliation incorporates: public response uses `workspaceUpdatedAt` (Workspace last-modified time, not exclusive Canvas timestamp); Element object embeds current public display metadata (not immutable snapshot); total payload ceiling `MAX_CANVAS_SNAPSHOT_BYTES = 524288` (512 KB) with authorized NestJS Express body-parser 1 MB limit in `src/main.ts`.
...
- **Canvas Graph Snapshot Contract**: Frozen & reconciled in `docs/ai/plans/CANVAS_SNAPSHOT_API_PLAN.md` (`FROZEN_CANVAS_SNAPSHOT_API_CONTRACT_V1`). Defines `GET /workspaces/:id/canvas` (load canvas graph with embedded Element display object) and `PUT /workspaces/:id/canvas` (replace complete graph atomically via `WorkspaceCanvasService`). Response uses `workspaceUpdatedAt` (Workspace last-modified time), Element object contains current public display metadata, total payload max 512 KB (`MAX_CANVAS_SNAPSHOT_BYTES = 524288`) with authorized 1 MB body-parser limit in `main.ts`. Source implementation not started.
