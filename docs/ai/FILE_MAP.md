...
│   │   │   ├── workspace-canvas.service.ts # Canvas snapshot GET & PUT service
...
- `fincraft-lab-api/src/workspace/workspace-canvas.service.ts` — Owns Canvas Snapshot `getSnapshot` (read-only current master element display metadata join) and `saveSnapshot` (atomic Prisma `$transaction` graph replacement, status/ownership check, element availability check, global ID collision check, aggregate 512 KB size limit check).
