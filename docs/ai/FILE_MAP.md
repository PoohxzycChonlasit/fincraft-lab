...
| Path | Responsibility | Origin | Next step allowed to change it |
|---|---|---|---|
| `package.json` | Dependencies (`bcrypt`, `class-validator`, `class-transformer`), scripts (`test:e2e` uses `--experimental-vm-modules`), `packageManager: pnpm@11.15.1` | generated by `nest new`, Prisma/PostgreSQL added in P2A, Auth dependencies added in P4_AUTH_1 | Edit when a new dependency or script is needed |
...
| `.env.example` | Documented placeholder for `DATABASE_URL` pointing to `fincraft_lab` | created in P2B.1 | Update if new environment variables are introduced |
...
| `src/auth/auth.module.ts` | Imports `DatabaseModule`, registers `AuthController` and `AuthService` | created in P4_AUTH_1 | Edit when new auth features are added |
| `src/auth/auth.controller.ts` | Route prefix `auth`; `POST /auth/register` delegates to `AuthService` and returns `{ data: user }` | created in P4_AUTH_1 | Add login route in P4_AUTH_2 |
| `src/auth/auth.service.ts` | Hashes passwords asynchronously with `bcrypt`, creates `User` via `PrismaService`, handles duplicate email `P2002` error as `ConflictException` | created in P4_AUTH_1 | Add login logic in P4_AUTH_2 |
...
|---|---|---|
| `fincraft-lab-api/.env` | Local `DATABASE_URL` pointing to local PostgreSQL `fincraft_lab` database | Ignored via `.gitignore` (`.env` pattern); never committed |
