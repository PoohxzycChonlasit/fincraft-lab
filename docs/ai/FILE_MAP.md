...
| Path | Responsibility | Origin | Next step allowed to change it |
|---|---|---|---|
| `package.json` | Dependencies, scripts (`test:e2e` uses `--experimental-vm-modules`), `packageManager: pnpm@11.15.1` | generated by `nest new`, Prisma/PostgreSQL added in P2A, e2e script updated in P3A | Edit when a new dependency or script is needed |
...
| `prisma/migrations/<timestamp>_init_mvp_schema/migration.sql` | Initial DDL SQL migration file creating 12 enums, 15 tables, foreign keys, and indexes for MVP schema (unapplied create-only state) | created in P3B.1 | Apply in P3B.2 |
...
| `pnpm-workspace.yaml` | Stores pnpm's `allowBuilds` approval | generated | Edit when a new build script needs approval |
...
| `src/main.ts` | Entry point, app bootstrap | generated | Keep as-is until more config is needed |
| `src/app.module.ts` | Root module — imports `ConfigModule`, `DatabaseModule`, `HealthModule` | generated, updated in P1/P3A | Edit when next feature module is added |
