...
function EmphasisLabel({ emphasis = "normal" }: { emphasis?: FinancialLearningTapeRow["emphasis"] }) {
  if (emphasis === "strong") {
    return <span className="text-xs font-semibold text-[var(--color-teal-700)]">Key value</span>;
  }

  if (emphasis === "caution") {
    return <span className="text-xs font-semibold text-[var(--color-orange-700)]">Check input</span>;
  }
...
export function FinancialLearningTape({ title, rows, note }: FinancialLearningTapeProps) {
  return (
    <section className="border-y-2 border-[var(--color-teal-700)] py-5" aria-labelledby="financial-learning-tape-title">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        <h3 id="financial-learning-tape-title" className="text-xl font-semibold tracking-tight text-foreground">{title}</h3>
        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--color-teal-700)]">Illustrative learning only</p>
      </div>
