  ...
  return (
    <div className="flex min-w-0 items-center gap-2.5">
      <Link
        ...
        className="-m-1 min-w-0 rounded-lg p-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--ring)"
      >
        <FinCraftLogo />
      </Link>
...
export function ProductShell({ children, activeTab = "home", user, contentMode = "standard", layoutMode = "document" }: ProductShellProps) {
  ...
  const headerClass = isHome
    ? "product-header product-header--home fixed inset-x-0 top-0 z-40 h-(--shell-header-height) shrink-0 border-b transition-colors"
    : "product-header sticky top-0 z-40 h-(--shell-header-height) shrink-0 border-b border-(--border-subtle) bg-(--surface-solid) transition-colors";
  ...
  return (
    <div data-layout-mode={layoutMode} className={`flex min-w-0 flex-col bg-(--page-background) text-(--color-text-primary) ${isWorkspace ? "h-[100dvh] overflow-hidden" : "min-h-[100dvh]"}`}>
      ...
      {!isHome ? (
        <footer className="shrink-0 border-t border-(--border-subtle) bg-(--surface-solid) py-3 text-center text-xs font-medium text-muted-foreground">
