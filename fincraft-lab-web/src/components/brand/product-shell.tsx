...
export function ProductShell({ children, activeTab = "home", user, contentMode = "standard", layoutMode = "document" }: ProductShellProps) {
  ...
  const headerClass = isHome
    ? "product-header product-header--home fixed inset-x-0 top-0 z-40 h-[var(--shell-header-height)] shrink-0 border-b transition-colors"
  ...
  return (
    <div data-layout-mode={layoutMode} className={`flex min-w-0 flex-col bg-[var(--page-background)] text-[var(--color-text-primary)] ${isWorkspace ? "h-[100dvh] overflow-hidden" : "min-h-[100dvh]"}`}>
