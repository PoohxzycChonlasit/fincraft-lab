...
function AccountLink({ href, label, active, children }: { href: string; label: string; active?: boolean; children: ReactNode }) {
  return (
    <DropdownMenuItem asChild>
      <Link href={href} className={`flex min-h-11 w-full items-center gap-3 rounded-xl px-3 text-sm font-semibold outline-none transition-colors ${active ? "bg-(--surface-inset) text-foreground" : "text-muted-foreground hover:text-foreground"}`}>
        {children}
        <span>{label}</span>
      </Link>
    </DropdownMenuItem>
    ...
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        <button
          ...
          aria-expanded={open}
          className={`inline-flex min-h-11 max-w-[min(15rem,calc(100vw-7rem))] items-center gap-2 rounded-xl border px-2.5 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--focus-ring) focus-visible:ring-offset-2 ${activeAccount || open ? "border-(--border-selected) bg-(--surface-inset)" : "border-(--border-subtle) bg-(--surface-solid) hover:bg-(--surface-inset)"}`}
        >
          <Avatar size="sm" className="shrink-0 bg-(--surface-paper) font-bold text-(--color-action-primary)">
            {user.avatarUrl ? <AvatarImage src={user.avatarUrl} alt="" /> : null}
            <AvatarFallback>{getInitials(user.displayName)}</AvatarFallback>
          </Avatar>
          <span className="hidden min-w-0 flex-1 xl:block">
            <span className="block truncate text-xs font-bold text-foreground">{user.displayName}</span>
            <span className="block truncate text-[10px] font-bold uppercase tracking-wide text-(--color-text-secondary)">{user.role}</span>
          </span>
          <ChevronDown size={15} aria-hidden="true" className={`shrink-0 text-(--color-text-secondary) transition-transform ${open ? "rotate-180" : ""}`} />
        </button>
      </DropdownMenuTrigger>

      <DropdownMenuContent id="account-menu" align="end" className="w-72 max-w-[calc(100vw-1rem)] rounded-2xl border border-(--border-subtle) bg-(--surface-solid) p-2 shadow-(--shadow-overlay)">
        <DropdownMenuLabel className="px-3 py-2">
          ...
          <span className="mt-1 inline-flex rounded-full border border-(--border-subtle) bg-(--surface-inset) px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-(--color-action-primary)">{user.role}</span>
        </DropdownMenuLabel>
        <DropdownMenuSeparator />
