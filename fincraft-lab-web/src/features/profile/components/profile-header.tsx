...
export function ProfileHeader({ user, displayName, avatarPreview }: ProfileHeaderProps) {
  ...
  return (
    <section aria-label="Account identity summary" className="surface-paper rounded-2xl border border-(--border-subtle) p-4 shadow-xs sm:p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-center gap-4">
          <Avatar size="lg" className="h-16 w-16 rounded-2xl after:rounded-2xl">
            ...
            <AvatarFallback className="rounded-2xl bg-(--surface-inset) text-sm font-bold text-(--brand-primary)">
              {avatarPreview ? <UserRound aria-hidden="true" className="h-7 w-7" /> : initials(visibleName)}
            </AvatarFallback>
          </Avatar>
          <div className="min-w-0">
            <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-(--accent-orange)">Identity summary</p>
      ...
      <div className="mt-4 flex items-start gap-2 border-t border-(--border-subtle) pt-4 text-xs text-muted-foreground">
        <ShieldCheck aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0 text-(--accent-teal)" />
