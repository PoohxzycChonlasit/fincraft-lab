    ...
    <fieldset disabled={disabled} className="space-y-2">
      ...
      <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
        {SPECIES_OPTIONS.map((option) => {
          ...
          return <button key={option.value} type="button" aria-pressed={isSelected} onClick={() => onSelect(option.value)} className={`flex min-h-[68px] items-start gap-3 rounded-xl border p-3 text-left transition-colors ${isSelected ? "border-(--brand-accent) bg-(--surface-inset) ring-2 ring-(--brand-accent)" : "border-(--border-subtle) bg-(--surface-flat) hover:bg-(--surface-inset)"}`}><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-(--border-subtle) bg-(--surface-solid) text-(--brand-primary)"><SpeciesIcon species={option.value} className="h-5 w-5" /></span><span><span className="block text-xs font-bold text-foreground">{option.label}</span><span className="mt-0.5 block text-[11px] text-muted-foreground">{option.description}</span></span></button>;
        })}
      </div>
    </fieldset>
  );
...
function Field({ id, label, value, onChange, error, hint, placeholder, type = "text", maxLength = 200 }: { id: string; label: string; value: string; onChange: (value: string) => void; error?: string; hint?: string; placeholder: string; type?: "text" | "url"; maxLength?: number }) {
  ...
  return <div className="space-y-1.5"><label htmlFor={id} className="block text-xs font-semibold text-foreground">{label}</label><input id={id} type={type} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} maxLength={maxLength} aria-invalid={Boolean(error)} aria-describedby={error ? errorId : helpId} className={`min-h-[44px] w-full rounded-xl border bg-(--surface-flat) px-3.5 py-2.5 text-xs font-medium text-foreground transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--ring) ${error ? "border-destructive focus-visible:ring-destructive" : "border-(--border-subtle) focus-visible:border-(--brand-accent)"}`} />{(error || hint) && <p id={error ? errorId : helpId} className={`text-[11px] ${error ? "font-medium text-destructive" : "text-muted-foreground"}`}>{error || hint}</p>}</div>;
}
