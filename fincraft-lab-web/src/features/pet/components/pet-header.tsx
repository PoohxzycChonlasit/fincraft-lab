...
export function CompanionAvatar({ pet, className = "" }: { pet: CompanionAvatarData; className?: string }) {
  return (
    <Avatar className={`h-16 w-16 rounded-2xl after:rounded-2xl ${className}`}>
      ...
      <AvatarFallback className="rounded-2xl bg-[var(--surface-inset)] text-[var(--brand-primary)]">
...
function WelcomeBanner({ pet }: { pet: PetProfile }) {
  ...
  return (
    <div className="surface-paper flex items-start gap-2.5 rounded-xl border border-[var(--border-subtle)] p-3 text-xs text-foreground">
      <Sparkles aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0 text-[var(--accent-orange)]" />
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1 flex items-center gap-2">
              <span className="inline-flex items-center gap-1 rounded-full border border-[var(--border-subtle)] bg-[var(--surface-solid)] px-2.5 py-0.5 text-xs font-bold text-foreground">
