...
function SpeciesSelect({ value, onChange, disabled }: { value: PetSpecies; onChange: (species: PetSpecies) => void; disabled: boolean }) {
  return <div className="space-y-1.5"><label htmlFor="species" className="block text-xs font-semibold text-foreground">Species</label><select id="species" value={value} disabled={disabled} onChange={(event) => { const option = SPECIES_OPTIONS.find((item) => item.value === event.target.value); if (option) onChange(option.value); }} className="min-h-[44px] w-full rounded-xl border border-(--border-subtle) bg-(--surface-flat) px-3.5 py-2.5 text-xs font-medium text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--ring) focus-visible:border-(--brand-accent)">{SPECIES_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div>;
}
...
function Field({ id, label, value, onChange, error, hint, placeholder, type = "text", maxLength = 200, disabled }: { id: string; label: string; value: string; onChange: (value: string) => void; error?: string; hint?: string; placeholder?: string; type?: "text" | "url"; maxLength?: number; disabled: boolean }) {
  ...
  return <div className="space-y-1.5"><label htmlFor={id} className="block text-xs font-semibold text-foreground">{label}</label><input id={id} type={type} value={value} disabled={disabled} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} maxLength={maxLength} aria-invalid={Boolean(error)} aria-describedby={error ? errorId : hint ? helpId : undefined} className={`min-h-[44px] w-full rounded-xl border bg-(--surface-flat) px-3.5 py-2.5 text-xs font-medium text-foreground transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--ring) ${error ? "border-destructive focus-visible:ring-destructive" : "border-(--border-subtle) focus-visible:border-(--brand-accent)"}`} />{(error || hint) && <p id={error ? errorId : helpId} className={`text-[11px] ${error ? "font-medium text-destructive" : "text-muted-foreground"}`}>{error || hint}</p>}</div>;
}
