...
function OnboardingCard({ onCreated }: { onCreated: (pet: PetProfile) => void }) {
  return (
    <div className="surface-card w-full max-w-2xl space-y-6 rounded-2xl border border-[var(--border-subtle)] p-5 shadow-sm sm:p-7">
      <div className="space-y-2 border-b border-[var(--border-subtle)] pb-5">
        <div className="inline-flex items-center gap-1.5 rounded-full border border-[var(--border-subtle)] bg-[var(--surface-inset)] px-3 py-1 text-xs font-semibold text-foreground"><Sparkles aria-hidden="true" className="h-3.5 w-3.5 text-[var(--brand-accent)]" /><span>Companion onboarding</span></div>
