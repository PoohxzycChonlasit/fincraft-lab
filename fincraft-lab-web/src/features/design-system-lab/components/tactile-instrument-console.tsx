...
import { ConsoleControls, type AccentRole, type Density, type SurfaceMode, type VisualState } from "./console-controls";
import { ConsolePreview } from "./console-preview";

const RADIUS_MAP = [4, 8, 12, 16];
const SHADOW_MAP = [
  "none",
  "var(--shadow-resting)",
...
const ACCENT_COLOR_MAP: Record<AccentRole, { text: string; bg: string; border: string }> = {
  teal: { text: "text-[var(--color-teal-700)]", bg: "bg-[var(--color-teal-600)]", border: "border-[var(--color-teal-600)]" },
  orange: { text: "text-[var(--color-orange-700)]", bg: "bg-[var(--color-orange-600)]", border: "border-[var(--color-orange-600)]" },
  neutral: { text: "text-foreground", bg: "bg-foreground", border: "border-foreground" },
  destructive: { text: "text-[var(--destructive)]", bg: "bg-[var(--destructive)]", border: "border-[var(--destructive)]" },
};

const DENSITY_MAP: Record<Density, { padding: string; gap: string; textSize: string }> = {
  compact: { padding: "p-3", gap: "gap-2.5", textSize: "text-xs" },
...
export function TactileInstrumentConsole() {
  ...
  return (
    <div className="surface-flat border border-[var(--border-subtle)]">
      <div className="grid grid-cols-1 divide-y divide-[var(--border-subtle)] lg:grid-cols-12 lg:divide-x lg:divide-y-0">
        ...
        <ConsolePreview
          ...
          buttonStateClass={getButtonStateClass()}
        />
      </div>
    </div>
  );
