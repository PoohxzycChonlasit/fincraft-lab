...
function CategoryRecord({ category, isPending, onEdit, onArchive, onReactivate }: { category: AdminCategory; isPending: boolean; onEdit: () => void; onArchive: () => Promise<boolean>; onReactivate: () => Promise<boolean> }) {
  return <article role="listitem" className="grid gap-3 border-b border-[var(--border-subtle)] p-4 last:border-b-0 md:grid-cols-[minmax(0,1.4fr)_minmax(0,2fr)_8rem_9rem_auto] md:items-center md:gap-4">
...
function CategoryRecords({ categories, isPending, onEdit, onArchive, onReactivate }: { categories: AdminCategory[]; isPending: boolean; onEdit: (category: AdminCategory) => void; onArchive: (category: AdminCategory) => Promise<boolean>; onReactivate: (category: AdminCategory) => Promise<boolean> }) {
  ...
  return <div role="list" aria-label="Element Categories" className="surface-card overflow-hidden rounded-2xl"><div className="hidden border-b border-[var(--border-subtle)] bg-[var(--surface-flat)] px-4 py-3 text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground md:grid md:grid-cols-[minmax(0,1.4fr)_minmax(0,2fr)_8rem_9rem_auto] md:gap-4"><span>Category</span><span>Description</span><span>Elements</span><span>Updated</span><span className="text-right">Actions</span></div>{categories.map((category) => <CategoryRecord key={category.id} category={category} isPending={isPending} onEdit={() => onEdit(category)} onArchive={() => onArchive(category)} onReactivate={() => onReactivate(category)} />)}</div>;
}
...
export function CategoryManagementClient({ initialCategories, initialLoadError = null }: CategoryManagementProps) {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-end justify-between gap-3 border-b border-[var(--border-subtle)] pb-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--color-craft-accent)]">Content catalogue</p><h1 className="mt-1 text-2xl font-bold tracking-tight text-foreground">Categories <span className="font-mono text-sm font-normal text-muted-foreground">{state.categories.length}</span></h1><p className="mt-1 max-w-2xl text-xs leading-5 text-muted-foreground">Organize shared Elements without silently detaching their stored relationships.</p></div><button id="new-category-button" type="button" onClick={() => state.setActiveModalCat("new")} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[var(--color-action-primary)] px-4 text-xs font-semibold text-white hover:bg-[var(--color-action-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)]"><Plus className="size-4" aria-hidden="true" />New Category</button></div>
