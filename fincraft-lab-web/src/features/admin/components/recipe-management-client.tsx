...
function RecipeExpression({ recipe }: { recipe: AdminRecipe }) {
  ...
  return <div className="flex min-w-0 flex-wrap items-center gap-2 text-sm font-semibold text-foreground" aria-label="Recipe relationship"><span className="break-words">{(inputA?.emoji ? inputA.emoji + " " : "") + (inputA?.name || "Input A")}</span><span className="text-[var(--color-craft-accent)]" aria-hidden="true">+</span><span className="break-words">{(inputB?.emoji ? inputB.emoji + " " : "") + (inputB?.name || "Input B")}</span><ArrowRight className="size-4 shrink-0 text-muted-foreground" aria-hidden="true" /><span className="break-words text-[var(--brand-primary)]">{(output?.emoji ? output.emoji + " " : "") + (output?.name || "Output")}</span></div>;
}
...
function RecipeRecord({ recipe, isPending, onEdit, onArchive, onReactivate }: { recipe: AdminRecipe; isPending: boolean; onEdit: () => void; onArchive: () => Promise<boolean>; onReactivate: () => Promise<boolean> }) {
  ...
  return <article role="listitem" className="grid gap-3 border-b border-[var(--border-subtle)] p-4 last:border-b-0 md:grid-cols-[minmax(0,2fr)_12rem_9rem_auto] md:items-center md:gap-4">
    ...
    <div className="flex flex-wrap justify-start gap-2 md:justify-end">
      <button id={"edit-recipe-" + recipe.id} type="button" onClick={onEdit} className="inline-flex min-h-11 items-center gap-1 rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-inset)] px-3 text-xs font-semibold text-foreground hover:bg-[var(--surface-raised)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)]"><Edit2 className="size-4" aria-hidden="true" />Edit</button>
      {canArchive ? <AdminArchiveDialog trigger={<button type="button" disabled={isPending} className="min-h-11 rounded-lg border border-amber-500/30 px-3 text-xs font-semibold text-amber-700 hover:bg-amber-500/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/50 disabled:opacity-50">Archive</button>} itemName="this Craft Recipe" description="This changes the Recipe status to INACTIVE. Its input and output relationships remain stored, and the Recipe can be reactivated later." isPending={isPending} onConfirm={onArchive} /> : <button type="button" onClick={() => void onReactivate()} disabled={isPending} className="inline-flex min-h-11 items-center gap-1 rounded-lg border border-[var(--border-interactive)] px-3 text-xs font-semibold text-[var(--brand-primary)] hover:bg-[var(--surface-inset)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)] disabled:opacity-50"><RotateCcw className="size-4" aria-hidden="true" />Reactivate</button>}
    </div>
  </article>;
}
...
function RecipeRecords({ recipes, isPending, onEdit, onArchive, onReactivate }: { recipes: AdminRecipe[]; isPending: boolean; onEdit: (recipe: AdminRecipe) => void; onArchive: (recipe: AdminRecipe) => Promise<boolean>; onReactivate: (recipe: AdminRecipe) => Promise<boolean> }) {
  ...
  return <div role="list" aria-label="Craft Recipes" className="surface-card overflow-hidden rounded-2xl"><div className="hidden border-b border-[var(--border-subtle)] bg-[var(--surface-flat)] px-4 py-3 text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground md:grid md:grid-cols-[minmax(0,2fr)_12rem_9rem_auto] md:gap-4"><span>Relationship</span><span>Rule type</span><span>Updated</span><span className="text-right">Actions</span></div>{recipes.map((recipe) => <RecipeRecord key={recipe.id} recipe={recipe} isPending={isPending} onEdit={() => onEdit(recipe)} onArchive={() => onArchive(recipe)} onReactivate={() => onReactivate(recipe)} />)}</div>;
}
...
export function RecipeManagementClient({ initialRecipes, initialLoadError = null }: RecipeManagementProps) {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-end justify-between gap-3 border-b border-[var(--border-subtle)] pb-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--color-craft-accent)]">Content catalogue</p><h1 className="mt-1 text-2xl font-bold tracking-tight text-foreground">Craft Recipes <span className="font-mono text-sm font-normal text-muted-foreground">{state.recipes.length}</span></h1><p className="mt-1 max-w-2xl text-xs leading-5 text-muted-foreground">Manage the rules that connect exactly two Elements to one resulting Element.</p></div><button id="new-recipe-button" type="button" onClick={() => state.setActiveModalRecipe("new")} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[var(--color-action-primary)] px-4 text-xs font-semibold text-white hover:bg-[var(--color-action-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)]"><Plus className="size-4" aria-hidden="true" />New Recipe</button></div>
    ...
    {state.isLoading ? <div className="surface-inset rounded-2xl p-8 text-center" aria-busy="true"><p className="text-sm font-semibold text-foreground">Loading Craft Recipes...</p></div> : state.loadError ? <div role="alert" className="surface-inset rounded-2xl border border-destructive/30 p-6"><p className="text-sm font-semibold text-destructive">Craft Recipes could not be loaded</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{state.loadError}</p><button type="button" onClick={() => void state.retryLoad()} className="mt-4 min-h-11 rounded-xl bg-[var(--color-action-primary)] px-4 text-xs font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)]">Retry</button></div> : <RecipeRecords recipes={state.recipes} isPending={state.isPending} onEdit={state.setActiveModalRecipe} onArchive={state.handleArchive} onReactivate={state.handleReactivate} />}
