...
function CategoryFormFields({ form, errors, update, onSubmit }: { form: FormState; errors: FieldErrors; update: <K extends keyof FormState>(key: K, value: FormState[K]) => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return <form id="category-form" className="space-y-4" onSubmit={onSubmit}>
    <div className="space-y-2"><FieldLabel htmlFor="category-name">Category name *</FieldLabel><input id="category-name" required minLength={2} maxLength={100} value={form.name} onChange={(event) => update("name", event.target.value)} aria-invalid={Boolean(errors.name)} aria-describedby={errors.name ? "category-name-error" : undefined} className="min-h-11 w-full rounded-xl border border-(--border-subtle) bg-(--surface-paper) px-3 py-2.5 text-sm text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--ring) aria-invalid:border-destructive" /><FieldError id="category-name-error" message={errors.name} /></div>
    <div className="space-y-2"><FieldLabel htmlFor="category-description">Description</FieldLabel><textarea id="category-description" maxLength={500} rows={4} value={form.description} onChange={(event) => update("description", event.target.value)} className="w-full resize-y rounded-xl border border-(--border-subtle) bg-(--surface-paper) px-3 py-2.5 text-sm leading-6 text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--ring)" /></div>
    <div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><FieldLabel htmlFor="category-sort">Sort order</FieldLabel><input id="category-sort" type="number" min={0} value={form.sortOrder} onChange={(event) => update("sortOrder", Number(event.target.value) || 0)} aria-invalid={Boolean(errors.sortOrder)} aria-describedby={errors.sortOrder ? "category-sort-error" : undefined} className="min-h-11 w-full rounded-xl border border-(--border-subtle) bg-(--surface-paper) px-3 py-2.5 text-sm text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--ring) aria-invalid:border-destructive" /><FieldError id="category-sort-error" message={errors.sortOrder} /></div><div className="space-y-2"><FieldLabel htmlFor="category-status">Status</FieldLabel><select id="category-status" value={form.status} onChange={(event) => update("status", event.target.value as FormState["status"])} className="min-h-11 w-full rounded-xl border border-(--border-subtle) bg-(--surface-paper) px-3 py-2.5 text-sm text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--ring)"><option value="ACTIVE">ACTIVE</option><option value="INACTIVE">INACTIVE</option></select></div></div>
  </form>;
}
...
function CategoryFormDialog({ form, errors, error, isPending, isEditing, onClose, onSubmit, update }: { form: FormState; errors: FieldErrors; error: string | null; isPending: boolean; isEditing: boolean; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; update: <K extends keyof FormState>(key: K, value: FormState[K]) => void }) {
  return <Dialog open onOpenChange={(open) => { if (!open && !isPending) onClose(); }}>
    <DialogContent className="flex max-h-[calc(100dvh-1rem)] max-w-lg flex-col gap-0 overflow-hidden p-0">
      <DialogHeader className="shrink-0 border-b border-(--border-subtle) p-5 pr-14"><DialogTitle>{isEditing ? "Edit Category" : "Create Category"}</DialogTitle><DialogDescription>{isEditing ? "Update this taxonomy record while keeping its Element relationships intact." : "Create a precise taxonomy record for the shared Element catalogue."}</DialogDescription></DialogHeader>
      ...
      <DialogFooter className="mx-0 mb-0 shrink-0"><button type="button" onClick={onClose} disabled={isPending} className="min-h-11 rounded-xl border border-(--border-subtle) bg-(--surface-inset) px-4 text-xs font-semibold text-foreground hover:bg-(--surface-raised) focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--focus-ring) disabled:opacity-60">Cancel</button><button type="submit" form="category-form" disabled={isPending} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-(--color-action-primary) px-5 text-xs font-semibold text-white hover:bg-(--color-action-hover) focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--focus-ring) disabled:opacity-60">{isPending ? <Loader2 className="size-4 animate-spin" aria-hidden="true" /> : null}{isPending ? "Saving..." : isEditing ? "Save changes" : "Create Category"}</button></DialogFooter>
    </DialogContent>
  </Dialog>;
}
