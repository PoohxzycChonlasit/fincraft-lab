...
function TextField({ field, value, onChange }: { field: LongField; value: string | null; onChange: (value: string) => void }) {
  return <div className="space-y-2"><label htmlFor={"detail-" + field.key} className="text-xs font-semibold text-foreground">{field.label}{field.required ? " *" : ""}</label><textarea id={"detail-" + field.key} required={field.required} maxLength={field.maxLength} value={value ?? ""} onChange={(event) => onChange(event.target.value)} rows={field.required ? 4 : 3} className="w-full resize-y rounded-xl border border-(--border-subtle) bg-(--surface-paper) px-3 py-2.5 text-sm leading-6 text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--ring)" /></div>;
}
...
export function AdminElementDetailEditor({ elementId, elementName, onClose, onSaved }: DetailEditorProps) {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open && !editor.isSaving) onClose(); }}><DialogContent className="flex max-h-[calc(100dvh-1rem)] max-w-4xl flex-col gap-0 overflow-hidden p-0"><DialogHeader className="shrink-0 border-b border-(--border-subtle) p-5 pr-14"><DialogTitle>Discovery Detail · {elementName}</DialogTitle><DialogDescription>Maintain the educational lesson, safety metadata, Recipe relationships, and authoritative Sources for this Element.</DialogDescription></DialogHeader><div className="min-h-0 flex-1 overflow-y-auto"><EditorBody editor={editor} /></div>{!editor.isLoading && !editor.loadError ? <DialogFooter className="mx-0 mb-0 shrink-0"><button type="submit" form="admin-element-detail-form" disabled={editor.isSaving} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-(--color-action-primary) px-5 text-xs font-semibold text-white hover:bg-(--color-action-hover) focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--focus-ring) disabled:opacity-60">{editor.isSaving ? <Loader2 className="size-4 animate-spin" aria-hidden="true" /> : <Save className="size-4" aria-hidden="true" />}{editor.isSaving ? "Saving..." : "Save Discovery Detail"}</button></DialogFooter> : null}</DialogContent></Dialog>;
}
