...
function ElementFormBody({ form, errors, categories, isEditing, isLoadingCategories, categoryError, onChange, onSubmit }: { form: ElementFormValues; errors: ElementFieldErrors; categories: CategoryOption[]; isEditing: boolean; isLoadingCategories: boolean; categoryError: string | null; onChange: <K extends keyof ElementFormValues>(key: K, value: ElementFormValues[K]) => void; onSubmit: (event: React.FormEvent<HTMLFormElement>) => void }) {
  ...
  return <div className="min-h-0 flex-1 overflow-y-auto p-5">
    {isLoadingCategories ? <div role="status" aria-busy="true" className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-inset)] p-4 text-xs text-muted-foreground">Loading Category options...</div> : null}
...
function ElementFormDialog({ form, errors, error, categories, isEditing, isSubmitting, isLoadingCategories, categoryError, onCancel, onChange, onSubmit }: { form: ElementFormValues; errors: ElementFieldErrors; error: string | null; categories: CategoryOption[]; isEditing: boolean; isSubmitting: boolean; isLoadingCategories: boolean; categoryError: string | null; onCancel: () => void; onChange: <K extends keyof ElementFormValues>(key: K, value: ElementFormValues[K]) => void; onSubmit: (event: React.FormEvent<HTMLFormElement>) => void }) {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open && !isSubmitting) onCancel(); }}>
    <DialogContent className="flex max-h-[calc(100dvh-1rem)] max-w-3xl flex-col gap-0 overflow-hidden p-0">
      <DialogHeader className="shrink-0 border-b border-[var(--border-subtle)] p-5 pr-14"><DialogTitle>{isEditing ? "Edit Element" : "Create Element"}</DialogTitle><DialogDescription>{isEditing ? "Update the editable master fields. Slug, type, and starter status stay fixed after creation." : "Create a shared master Element for the learning catalogue."}</DialogDescription></DialogHeader>
      ...
      <DialogFooter className="mx-0 mb-0 shrink-0"><button type="button" onClick={onCancel} disabled={isSubmitting} className="min-h-11 rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-inset)] px-4 text-xs font-semibold text-foreground hover:bg-[var(--surface-raised)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)] disabled:opacity-60">Cancel</button><button type="submit" form="admin-element-form" disabled={isSubmitting || categoryUnavailable || isLoadingCategories} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-[var(--color-action-primary)] px-5 text-xs font-semibold text-white hover:bg-[var(--color-action-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)] disabled:opacity-60">{isSubmitting ? "Saving..." : isEditing ? "Save changes" : "Create Element"}</button></DialogFooter>
    </DialogContent>
  </Dialog>;
}
