...
function FormFooter({ isSubmitting, isEditing, onCancel }: { isSubmitting: boolean; isEditing: boolean; onCancel: () => void }) {
  return (
    <div className="flex justify-end gap-3 pt-2">
      ...
      <button type="submit" disabled={isSubmitting} className="min-h-[44px] rounded-xl bg-[var(--color-action-primary)] text-white px-5 py-2 text-xs font-semibold hover:bg-[var(--color-action-hover)] disabled:opacity-60">
...
export function AdminElementForm({ initialElement, categories, isSubmitting, onCancel, onSubmitCreate, onSubmitUpdate }: FormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="surface-inset rounded-2xl border border-[var(--border-subtle)] p-5 space-y-4">
      ...
      <FormFooter isSubmitting={isSubmitting} isEditing={isEditing} onCancel={onCancel} />
    </form>
  );
}
