  ...
  return (
    <article role="listitem" className="grid gap-4 border-b border-[var(--border-subtle)] p-4 last:border-b-0 md:grid-cols-[minmax(0,1.7fr)_minmax(10rem,1fr)_minmax(12rem,1.2fr)_auto] md:items-center">
      ...
      <div className="flex flex-wrap items-center justify-start gap-2 md:justify-end">
        <button id={`inspect-element-${element.id}`} type="button" onClick={onOpenDetail} className="inline-flex min-h-11 items-center gap-1 rounded-lg border border-[var(--border-interactive)] px-3 text-xs font-semibold text-[var(--brand-primary)] hover:bg-[var(--surface-inset)]"><Eye className="size-4" aria-hidden="true" />Inspect detail</button>
        <button type="button" onClick={onEdit} className="inline-flex min-h-11 items-center gap-1 rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-inset)] px-3 text-xs font-semibold text-foreground hover:bg-[var(--surface-raised)]"><Pencil className="size-4" aria-hidden="true" />Edit</button>
        {canArchive ? <AdminArchiveDialog trigger={<button type="button" disabled={isSubmitting} className="inline-flex min-h-11 items-center gap-1 rounded-lg border border-amber-500/30 px-3 text-xs font-semibold text-amber-700 hover:bg-amber-500/10 disabled:opacity-50">Archive</button>} itemName={element.name} description="This Element remains stored, but it becomes unavailable to normal discovery and stays visible in Admin as inactive content." isPending={isSubmitting} onConfirm={onArchive} /> : <button type="button" onClick={() => void onReactivate()} disabled={isSubmitting} className="inline-flex min-h-11 items-center gap-1 rounded-lg border border-[var(--border-interactive)] px-3 text-xs font-semibold text-[var(--brand-primary)] hover:bg-[var(--surface-inset)] disabled:opacity-50"><RotateCcw className="size-4" aria-hidden="true" />Reactivate</button>}
      </div>
    </article>
  );
}
